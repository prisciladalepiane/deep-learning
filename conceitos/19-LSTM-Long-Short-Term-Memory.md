...
Uma rede LSTM é composta por uma série de células LSTM, cada uma das quais é responsável por manter um estado ao longo do tempo. Cada célula tem três portões: o portão de entrada (input gate), o portão de esquecimento (forget gate) e o portão de saída (output gate).
